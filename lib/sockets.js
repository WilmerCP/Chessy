...
socketManager.handler = function (socket) {
    ...
    socket.on('findgame', (data) => {
        ...
        if (socketManager.queue.length >= 2) {
            ...
            if (randomNum == 1) {
                ...
                let startTime = gameManager.initGame(roomid, player2.id, player1.id);

                socketManager.io.to(player1.id).emit('startGame', { 'opponent': player2.name, 'color': 'black', 'startTime' : startTime});
                socketManager.io.to(player2.id).emit('startGame', { 'opponent': player1.name, 'color': 'white', 'startTime' : startTime});

            } else {

                let startTime = gameManager.initGame(roomid, player1.id, player2.id);

                socketManager.io.to(player1.id).emit('startGame', { 'opponent': player2.name, 'color': 'white', 'startTime' : startTime});
                socketManager.io.to(player2.id).emit('startGame', { 'opponent': player1.name, 'color': 'black', 'startTime' : startTime});
    ...
    gameManager.timeoutEmitter.on('timeout', (data) => {

        socketManager.io.to(data.matchId).emit('gameover', {'reason': 'time-out', 'winner': data.winner})
    ...
    socket.on('move', (data) => {
        ...
        if (gameManager.moveRequest(data) && room) {
            ...
            let time = gameManager.getTimerStart(room);

            if(castle){

                socketManager.io.to(room).emit('castle',{'square':castle, 'startTime': time});
    ...
    socket.on('disconnect', () => {
        ...
        const rooms = socketManager.io.sockets.adapter.rooms;
        //console.log('All rooms:', rooms);

    
        rooms.forEach((sockets, roomName) => {

            //console.log(`Room: ${roomName}, Sockets:`, sockets);

            if(roomName.indexOf(socket.id) > -1){

                let winner = gameManager.resignation({ matchId: roomName, userId: socket.id});
    
                if(winner){
    
                    socketManager.io.to(roomName).emit('gameover', {'reason': 'resignation', 'winner': winner})
