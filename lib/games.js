...
//If the king is not in check, updates the state of the game and returns true
...
gameManager.evaluateMove = function (data) {
    ...
    if (gameManager.validColor(matchId, from, to, playerId)) {
        ...
        if (movedPiece.indexOf('peon') != -1 && gameManager.pawnMove(matchId, from, to)) {
            ...
            return gameManager.updateState(matchId, from, to);
            ;
...
gameManager.canRunAway = function(board,kingSquare,color){
    ...
    if (rank < 7) {
        ...
        if (gameManager.isAvailable(board,gameManager.getSquareName(f+1,r+1),color)) {
...
gameManager.checkmate = function (matchId) {
    ...
    console.log('The king is at ' + kingSquare);

    console.log('The king is in check ' + gameManager.squareInCheck(board, kingSquare, state.next));
